...
const SequelizeStorage = require('umzug/lib/storages/SequelizeStorage').default;

[
  'db:migrate --use-transaction',
  ...
  const prepare = function (callback, options) {
    ...
    migrationFile = 'transactional/*' + migrationFile + '.js';
  ...
  describe(Support.getTestDialectTeaser(flag), () => {
    ...
    xit('doesnt do anything if logmigrate fails', function (done) {

      const sandbox = sinon.createSandbox();

      sinon.stub(SequelizeStorage.prototype, 'logMigration').throws()
      const self = this;
    ...
    describe('promise based migrations', () => {
      it('correctly creates two tables', function (done) {
        ...
        prepare(() => {
          ...
          migrationFile: 'createPersonAndTask'
        });
      });
    });
    ...
    describe('custom meta table name', () => {
      it('correctly uses the defined table name', function (done) {
        ...
        prepare(() => {
          helpers.readTables(self.sequelize, tables => {
            expect(tables.sort()).to.eql(['Person', 'sequelize_meta']);
            done();
          });
        }, {
          ...
          if (Support.dialectIsPostgres()) {
            helpers.readSchemas(this.sequelize, schemas => {
              ...
              helpers.readTables(this.sequelize, tables => {
                expect(tables.sort()).to.eql(['Person']);
                done();
              });
            });
          } else {
            ...
            helpers.readTables(this.sequelize, tables => {
              expect(tables.sort()).to.eql(['Person', 'sequelize_schema.SequelizeMeta']);
              done();
            });
          }
        }, {
...
xdescribe(Support.getTestDialectTeaser('db:migrate'), () => {
